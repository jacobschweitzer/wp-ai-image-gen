...
const createRepository = ( t ) => {
	...
	mkdirSync( path.join( repository, 'secondary/build' ), {
		recursive: true,
	} );
	writeFileSync( path.join( repository, 'build/index.js' ), 'tracked build' );
	writeFileSync(
		path.join( repository, 'secondary/build/index.js' ),
		'tracked secondary build'
	);
...
const buildPaths = [ 'build/', 'secondary/build/' ];
