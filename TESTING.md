...
matching one set of mocks. This document covers the base plugin.
...
| JavaScript coverage   | Risk-selected module thresholds remain satisfied                       | `npm run test:unit:coverage`                                                     | Tests                 |
...
The JavaScript mutation gate is 98%. One equivalent mutation survives: changing
`id > 0` to `id >= 0` cannot change the result because the preceding truthiness
check already rejects zero. Keep this visible rather than excluding the check.

## Local prerequisites

- PHP coverage and Infection require Xdebug. CI installs it automatically.
- Infection is installed separately by CI with PHP 8.3; install its CLI
  separately to run the optional PHP mutation report locally.
